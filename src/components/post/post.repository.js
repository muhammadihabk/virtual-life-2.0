...
module.exports.getPostByIdRepository = async function getPostByIdRepository(
  ...
  try {
    ...
    const count_replies_query = knexClient
      .queryBuilder()
      .select([
        `${Table.COMMENT}.${Comment.PARENT_COMMENT_ID}`,
        knexClient.raw(
          `COUNT(${Table.COMMENT}.${Comment.ID}) AS count_replies`
        ),
      ])
      .from(Table.COMMENT)
    ...
    const comments = await knexClient
      ...
      .leftJoin(
        ...
        `replies_counts.${Comment.PARENT_COMMENT_ID}`
      )
      .where({
        [Comment.POST_ID]: postId,
      });
...
module.exports.getHomefeedPaginateRepository =
  async function getHomefeedPaginateRepository(userId, paginateOptions) {
    const limit = paginateOptions.limit || SearchDefaultLimit;
    const offset = paginateOptions.offset || SearchDefaultOffset;
