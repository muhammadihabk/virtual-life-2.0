const { Table, Comment, User, Post } = require('../db.enums');
...
exports.up = function (knex) {
  return knex.schema.createTable(Table.COMMENT, (table) => {
    ...
    table.foreign(Comment.AUTHOR_ID).references(User.ID).inTable(Table.USER).onDelete('CASCADE');
    table.foreign(Comment.POST_ID).references(Post.ID).inTable(Table.POST).onDelete('CASCADE');
